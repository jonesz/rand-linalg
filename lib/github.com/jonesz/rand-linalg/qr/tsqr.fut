...
module structured_thin_qr (D: real) = {
    module L = mk_linalg D

    def house [m] (x: [m]D.t) =
      -- If floating point, the sign bit should be flipped on `x[0]`.
      -- TODO: Need to figure out this bug; I tested against a GS and it seemed
      -- that the bit was flipped.
      let a =
    ...
    def qr [n][m] (A: [m][n]D.t) : ([m][n]D.t, [n][n]D.t) =
      let Q_w ip : [m][n]D.t = (L.eye n) ++ (replicate (m-n) (replicate n (D.i64 0))) :> [m][n]D.t
      let (Q, R) =
         loop (Q, R) = (Q_wip, A) for k < n do
...
module TSQR (D: real)
  ...
  module QR_E = structured_thin_qr D
